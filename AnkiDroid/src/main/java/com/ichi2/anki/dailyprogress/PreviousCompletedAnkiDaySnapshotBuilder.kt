...
class PreviousCompletedAnkiDaySnapshotBuilder {
    fun build(collection: Collection): PreviousCompletedAnkiDaySnapshot {
        val currentDayCutoffEpochMs = collection.sched.dayCutoff * 1000L
        val windowEndEpochMsExclusive = currentDayCutoffEpochMs - 1.days.inWholeMilliseconds
        val windowStartEpochMs = windowEndEpochMsExclusive - 1.days.inWholeMilliseconds
        ...
        collection.db.query(query, windowStartEpochMs, windowEndEpochMsExclusive).use { cursor ->
            check(cursor.moveToFirst()) { "Expected aggregate row for previous completed Anki day snapshot" }
            val answerEventsTotal = cursor.getInt(1)
            return PreviousCompletedAnkiDaySnapshot(
                ...
                answerButtons =
                    PreviousCompletedAnkiDaySnapshot.AnswerButtons(
                ...
                reviewKinds =
                    PreviousCompletedAnkiDaySnapshot.ReviewKinds(
